...
if (strlen($_SESSION['pgasuid']==0)) {
  ...
  $comaddress=$_POST['comaddress'];
	...
	$alternetnumber=$_POST['alternetnumber'];

     $query=mysqli_query($con, "update tbluser set FullName ='$fullname', Email='$email', MobileNumber='$mobno',FatherName='$fathername',dob='$dob',ComAddress='$comaddress',EmergencyNumber='$emergencynumber',AlternetNumber='$alternetnumber' where ID='$uid'");
    if ($query) {
...
<?php } ?>
							...
							<div class="form-group">
								<input type="date" class="form-control" id="dob" name="dob" placeholder="Date of Birth" onfocus="(this.type='date')" onblur="if(!this.value)this.type='text'"required='true' value="<?php  echo $row['dob'];?>">
							</div>
							<div class="form-group">
					...
					<!--	<div class="form-group">
								...
								<input type="text" class="form-control" id="alternatenumber" name="alternatenumber" placeholder="Alternate Number" value="<?php  echo $row['AlternetNumber'];?>">
							</div>	-->
						</div>
						<?php } ?>
